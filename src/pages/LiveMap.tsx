...
export default function LiveMap() {
  ...
  const pollData = useCallback(async () => {
    try {
      ...
      // Build floors dynamically from floor_id
      const floorIds = new Set<string>();
      positions.forEach((p) => floorIds.add(p.floor_id));
      apiAnchors.forEach(() => {}); // anchors may not have floor info
      ...
      // Map anchors → anchor devices (place on first floor if no floor info)
      ...
      // Auto-enable all floors on first load
  ...
  const mapCenter: [number, number] = useMemo(() => {
    const totalH =
      Object.values(floorOffsets).length > 0
  ...
  return (
    <div className="h-full flex">
      ...
      <div className="w-48 border-r border-border bg-card p-3 flex flex-col gap-3 overflow-y-auto shrink-0">
        ...
        {floors.length === 0 && (
          <div className="text-xs text-muted-foreground italic">Waiting for data…</div>
        )}
      ...
      <div className="flex-1 relative">
        ...
        >
          ...
          {floors
            .filter((f) => enabledFloors.has(f.id))
            .map((floor) => {
          ...
          <Card className="absolute bottom-4 left-4 p-3 max-w-xs z-[1000]">
            ...
            <div className="text-xs space-y-1">
              ...
              <div className="text-muted-foreground">
                Floor: {floors.find((f) => f.id === searchResult.floorId)?.floor ?? '—'}, Room{' '}
                {searchResult.room}
              </div>
